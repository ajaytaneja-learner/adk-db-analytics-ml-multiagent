...
#     # ],
#     if NL2SQL_METHOD == "CHASE":
#         nl2sql_tool = chase_db_tools.initial_bq_nl2sql
#     else:
#     nl2sql_tool = tools.initial_bq_nl2sql

# tools = [nl2sql_tool, bigquery_toolset]
...
# Decide which NL2SQL tool to use based on environment variable
if NL2SQL_METHOD == "CHASE":
    nl2sql_tool = chase_db_tools.initial_bq_nl2sql
else:
    nl2sql_tool = tools.initial_bq_nl2sql
...
agent_tools = [
    nl2sql_tool,       # Natural language → SQL generator
    bigquery_toolset,  # Executes SQL against BigQuery
]

# Instantiate the BigQuery database agent
database_agent = Agent(
    model=os.getenv("BIGQUERY_AGENT_MODEL"),
    name="database_agent",
    instruction=return_instructions_bigquery(),
